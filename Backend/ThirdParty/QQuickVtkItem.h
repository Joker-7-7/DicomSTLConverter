/*=========================================================================
  ...
  Copyright (c)  John Stone

  See  https://github.com/john-stone-ics/QML-VTK-Examples/blob/main/HighlightPickedActor/QQuickVtkItem.h for details.
...
public:
    ...
    using Callback = std::function<void(vtkRenderWindow*, vtkUserData)>;

    /**
     * Forces the QQuickWindow with VTK render window to update instantly.
     *
     * @note Leads to call of #QQuickItem::forceActiveFocus() if the current item wasnt has been focused
     *       before the call.
     */
